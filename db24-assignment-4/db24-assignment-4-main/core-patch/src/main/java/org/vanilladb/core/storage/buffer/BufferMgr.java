...
public class BufferMgr implements TransactionLifecycleListener {
    ...
    public static double WAKE_RATIO = 0.3;  
	...
	public Buffer pin(BlockId blk) {
		...
		// Check buffer already pinned outside synchronized block
		...
		// Try to pin buffer, handling waiting outside the critical section
		synchronized (bufferPool) {
			buff = bufferPool.pin(blk);
		}
	
		if (buff == null) {
			synchronized (this) { // synchronize on a smaller scope for waitingThreads management
			...
			synchronized (this) { // minimize the scope of synchronization
			...
			if (buff == null) {
				repin();
				synchronized (bufferPool) {
					buff = bufferPool.pin(blk); // re-attempt to pin the buffer
				}
			}
		}
		...
		if (buff != null) {
			synchronized (bufferPool) {
				pinningBuffers.put(blk, new PinningBuffer(buff));
			}
		}
	...
	// 	boolean wasPinned = buffer.isPinned();
	// 	buffer.unpin();
	// 	if (wasPinned && !buffer.isPinned()) {
	// 		// Now that the buffer is unpinned, it might be available for other threads.
	// 		synchronized (bufferPool) {
	// 			bufferPool.notifyAll();
	// 		}
	// 	}
